...
#+BEGIN_SRC emacs-lisp
(fset 'mydb
   [?\C-x ?3 ?\C-x ?\C-f ?~ ?/ ?. ?e ?m tab ?. tab ?s ?t ?a ?r tab ?. ?o tab return ?\C-c ?a ?  ?\C-x ?- ?\C-x ?\C-- ?\C-x ?o ?\C-x ?2 ?\C-x ?\C-b])
#+END_SRC
* Python
...
(setq org-capture-templates
	...
	("T" "Task/Idea" entry (file+headline "~/jalihal_projects/Research/LabNoteBook.org" "Tasks")
	 "** TODO %?  %^g\n%u\n%a" )
